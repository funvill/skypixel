...
exports.svgOutputFolder = exports.svgOutput = void 0;
...
const CONFIG = {
    ...
    pointsOnARow: 40, // boxes per row (e.g., minutes per day / interval)
...
async function svgOutput(root) {
    console.log(`\n🔎 Generating svgOutput under ${root}\n`);
    ...
    for (const name of subdirs) {
        ...
        await renderSvgOutputFolder(folder, CONFIG);
    }
}
exports.svgOutput = svgOutput;
async function svgOutputFolder(folder) {
    await renderSvgOutputFolder(folder, CONFIG);
}
exports.svgOutputFolder = svgOutputFolder;
async function renderSvgOutputFolder(folder, config) {
    ...
    svg.push(`</svg>`);
    const outFile = path_1.default.join(folder, 'output.svg');
    await promises_1.default.writeFile(outFile, svg.join('\n'));
    console.log(`✅ Generated ${folder}/output.svg (${width}×${height})`);
}
