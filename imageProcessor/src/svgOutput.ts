...
export async function svgOutput(root: string) {
...
export async function svgOutputFolder(folder: string) {
  await renderSvgOutputFolder(folder, CONFIG);
}

async function renderSvgOutputFolder(folder: string, config: typeof CONFIG) {
    ...
    for (let i = 0; i < expectedCount; i++) {
      ...
      if (labelOffset) {
        svg.push(
          `<text x="${x + boxSize/2}" y="${y + boxSize/2 + 4}" ` +
    ...
    svg.push(`</svg>`);
    const outFile = path.join(folder, 'output.svg');
