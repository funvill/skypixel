...
type Args =
  | (Region & { file: string; settings?: undefined })
  | { settings: string; file?: undefined; x?: undefined; y?: undefined; width?: undefined; height?: undefined };

/**
 * Extracts the region from the source image and saves a preview prefixed with 'sky_'.
 */
async function saveRegionPreview(
  filePath: string,
  region: Region
): Promise<void> {
...
async function computeAverage(
  ...
  for (let i = 0; i < data.length; i += info.channels) {
    for (let c = 0; c < info.channels; c++) sums[c] += data[i + c];
  }
...
async function saveSvgBlocks(
  ...
  results.forEach((entry, idx) => {
    const { r, g, b, a } = entry.average as any;
  ...
  const svgPath = path.join(folder, 'output.svg');
  await fs.writeFile(svgPath, svg);
}

async function processSingle(file: string, region: Region) {
  try {
...
async function processBatch(settingsPath: string) {
  try {
    const content = await fs.readFile(settingsPath, 'utf-8');
    const entries: BatchEntry[] = JSON.parse(content);
    ...
    for (const entry of entries) {
      ...
      const results: { file: string; average: Record<string, number> }[] = [];
      const outJson = path.join(folder, 'output.json');
...
async function main() {
  const argv = yargs(hideBin(process.argv))
    ...
    .check(args => {
      if (args.settings) return true;
      const needed = ['file', 'x', 'y', 'width', 'height'];
      for (const opt of needed) {
        if (args[opt] === undefined) throw new Error(`--${opt} is required in single-file mode`);
      }
      return true;
    })
    .parseSync() as Args;
