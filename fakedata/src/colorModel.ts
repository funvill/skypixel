...
import { AverageColor, LocalDateTime, ProjectSettings, WeatherAtTime } from './types';
...
function mixColor(left: AverageColor, right: AverageColor, amount: number): AverageColor {
  return {
...
type ColorRegime = 'night' | 'twilight' | 'clear' | 'overcast' | 'mid';

function transitionMinutes(settings: ProjectSettings): number {
  return settings.generator.weather.transitionMinutes;
}

function averageLuminance(color: AverageColor): number {
  return 0.2126 * color.r + 0.7152 * color.g + 0.0722 * color.b;
}

function stableDayWeight(weather: WeatherAtTime, settings: ProjectSettings): number {
...
function totalMinutes(date: LocalDateTime): number {
  return date.hour * 60 + date.minute;
}

function absoluteDayIndex(date: LocalDateTime): number {
  return Math.floor(Date.UTC(date.year, date.month - 1, date.day, 12, 0, 0, 0) / 86400000);
}

function absoluteMinuteIndex(date: LocalDateTime): number {
  return Math.floor(utcTimestamp(date) / 60000);
}

function seasonalPosition(date: LocalDateTime): number {
  return (absoluteDayIndex(date) % 365) / 365;
}

function summerStrength(date: LocalDateTime): number {
  const distance = Math.abs(seasonalPosition(date) - 0.47);
  return clamp(1 - distance / 0.26, 0, 1);
}

function summerMiddayWeight(date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): number {
...
function edgeWindowWeight(weather: WeatherAtTime, edge: 'sunrise' | 'sunset', settings: ProjectSettings): number {
  const transition = transitionMinutes(settings);
  if (edge === 'sunrise') {
...
function bucketSeed(locationKey: string, date: LocalDateTime, bucketMinutes: number, suffix: string): string {
...
function continuousBucketSeed(locationKey: string, date: LocalDateTime, bucketMinutes: number, suffix: string): string {
...
function utcTimestamp(date: LocalDateTime): number {
  return Date.UTC(date.year, date.month - 1, date.day, date.hour, date.minute, 0, 0);
}

function moonIllumination(date: LocalDateTime): number {
...
function basePalette(weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const {
    preDawn,
    sunriseWarm,
    dayBlue,
...
function applyRadiation(color: AverageColor, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
...
function applyClouds(color: AverageColor, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const cloudFactor = clamp(weather.cloudCover / 100, 0, 1);
  const overcastTint: AverageColor = { r: 214, g: 220, b: 228 };
...
function applyMoisture(color: AverageColor, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const precipitationFactor = clamp((weather.precipitation + weather.rain + weather.snowfall) / 5, 0, 1);
  const stormTint: AverageColor = { r: 120, g: 132, b: 150 };
...
function applySnow(color: AverageColor, weather: WeatherAtTime): AverageColor {
  const snowFactor = clamp(weather.snowfall / 3, 0, 1);
  const snowTint: AverageColor = { r: 220, g: 229, b: 242 };
  return mixColor(color, snowTint, snowFactor * 0.42);
}

function applyTemperature(color: AverageColor, weather: WeatherAtTime): AverageColor {
...
function applyExposure(color: AverageColor, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const dayPlateau = stableDayWeight(weather, settings);
...
function regimeBlend(locationKey: string, date: LocalDateTime, windowDays: number, suffix: string): number {
...
function transitionPhaseWindow(weather: WeatherAtTime, edge: 'sunrise' | 'sunset', settings: ProjectSettings): number {
  return edgeWindowWeight(weather, edge, settings);
}

function transitionVisibility(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
...
function transitionTimingSkew(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
  const cloudFactor = clamp(weather.cloudCover / 100, 0, 1);
...
function transitionFeather(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset'): number {
...
function horizonTransmission(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
  const clearFactor = clamp(1 - weather.cloudCover / 100, 0, 1);
  const wetFactor = clamp((weather.precipitation + weather.rain + weather.snowfall) / 3.5, 0, 1);
...
function cloudBankOcclusion(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
  const cloudFactor = clamp(weather.cloudCover / 100, 0, 1);
  const wetFactor = clamp((weather.precipitation + weather.rain + weather.snowfall) / 3.5, 0, 1);
...
function transitionAmbientStrength(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
...
function transitionWarmBandStrength(locationKey: string, date: LocalDateTime, edge: 'sunrise' | 'sunset', weather: WeatherAtTime, settings: ProjectSettings): number {
  const phase = transitionPhaseWindow(weather, edge, settings);
...
function applySummerMiddayFlattening(color: AverageColor, date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  ...
  const cloudFactor = clamp(weather.cloudCover / 100, 0, 1);
  const grayBlueTarget: AverageColor = {
...
function applyTransitionFeathers(color: AverageColor, locationKey: string, date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
...
function applyOrganicVariation(color: AverageColor, locationKey: string, timestamp: string, date: LocalDateTime, weather: WeatherAtTime): AverageColor {
  ...
  if (weather.daylightRatio > 0) {
    ...
    if (overcastDayFactor > 0) {
      const overcastWhite: AverageColor = { r: 228, g: 230, b: 236 };
    ...
    if (blueSuppression > 0) {
      const gloomCloud: AverageColor = { r: 222, g: 226, b: 230 };
...
function classifyColorRegime(weather: WeatherAtTime, settings: ProjectSettings): ColorRegime {
  if (weather.daylightRatio === 0) {
...
function fitColorToRatios(color: AverageColor, targetRb: number, targetGb: number): AverageColor {
...
function applySeasonalWhiteBalance(color: AverageColor, locationKey: string, date: LocalDateTime, weather: WeatherAtTime): AverageColor {
...
function applyRegimeRatioFit(color: AverageColor, date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  ...
  let adjusted = mixColor(
    color,
    fitColorToRatios(color, lerp(0.6525, 0.8584, cloudBlend), lerp(0.7772, 0.8956, cloudBlend)),
    0.42
  );
  adjusted = {
    r: adjusted.r * 0.965,
...
function applyRealCameraBalance(color: AverageColor, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const cloudFactor = clamp(weather.cloudCover / 100, 0, 1);
...
function transitionStrength(weather: WeatherAtTime, settings: ProjectSettings): number {
  return Math.max(edgeWindowWeight(weather, 'sunrise', settings), edgeWindowWeight(weather, 'sunset', settings));
}

function applyWebcamResponse(color: AverageColor, locationKey: string, timestamp: string, date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): AverageColor {
  const twilight = transitionStrength(weather, settings);
...
function weatherCodePenalty(weatherCode: number): number {
  if (weatherCode >= 95) return 0.36;
  if (weatherCode >= 80) return 0.26;
  if (weatherCode >= 70) return 0.18;
  if (weatherCode >= 60) return 0.22;
  if (weatherCode >= 45) return 0.16;
  if (weatherCode >= 3) return 0.08;
  return 0;
}

export function generateSkyColor(locationKey: string, timestamp: string, date: LocalDateTime, weather: WeatherAtTime, settings: ProjectSettings): { average: AverageColor; confidence: number } {
  ...
  const seasonalSaturation = settings.generator.lighting.seasonalSaturationBase + clamp(
    Math.cos((date.month - 1) / 12 * Math.PI * 2) * settings.generator.lighting.seasonalSaturationAmplitude,
    -settings.generator.lighting.seasonalSaturationAmplitude,
    settings.generator.lighting.seasonalSaturationAmplitude
  );
  color = {
