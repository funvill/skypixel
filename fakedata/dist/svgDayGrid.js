...
exports.renderSvgDayGrid = renderSvgDayGrid;
...
function clampEdgeOffset(offsetColumns, earlyColumns, lateColumns) {
    return clamp(offsetColumns, earlyColumns, lateColumns);
}
function detectRowEdges(entries) {
...
function adjustEdgeColor(settings, dayKey, rowIndex, columnIndex, color, sunriseEdge, sunsetEdge) {
    ...
    const applyWarmGlow = (edge, edgeColumn, base) => {
        ...
        const texture = clamp(0.82
            + deterministicNoise(`${dayKey}:${edge}:${Math.floor(rowIndex / 3)}:${Math.floor(columnIndex / 2)}:texture-fine`) * 0.16
            + deterministicNoise(`${dayKey}:${edge}:${Math.floor(rowIndex / 7)}:${Math.floor(columnIndex / 6)}:texture-coarse`) * 0.12, 0.55, 1.18);
    ...
    const applyEdge = (edge, edgeColumn, base) => {
        const side = edge === 'sunrise' ? 1 : -1;
        const isSunset = edge === 'sunset';
        const jitterStrength = dayGridSettings.edgeJitterStrength;
        ...
        const afterglowTexture = clamp(0.82
            + deterministicNoise(`${dayKey}:sunset:${Math.floor(rowIndex / 4)}:${Math.floor(columnIndex / 3)}:afterglow`) * 0.18, 0.55, 1.05);
        const afterglowAmount = clamp(afterglowWeight * (0.34 + summerWeight * 0.14) * afterglowTexture * dayGridSettings.sunsetAfterglowIntensity, 0, 0.52);
...
async function renderSvgDayGrid(config) {
    ...
    const outputPath = path_1.default.resolve(config.outputFile ?? path_1.default.join(path_1.default.dirname(inputPath), 'day-grid.svg'));
    const days = new Map();
    for (const record of records) {
        const key = (0, svgData_1.recordDateKey)(record.timestamp);
    ...
    const title = (0, svgData_1.buildSvgTitle)(inputPath, 'day grid');
    ...
    svg.push(`<title>${(0, svgData_1.escapeXml)(title)}</title>`);
